---
title: "Delete Purchase Order"
api: "DELETE https://api.stateset.com/api/purchaseorder"
description: "This endpoint deletes an existing purchase order."
---

### Body

<ParamField body="purchase_order_id" type="string">
  The ID provided in the data tab may be used to identify the purchase order
</ParamField>

### Response

<ResponseField name="id" type="string">
  The ID provided in the data tab may be used to identify the purchase order
</ResponseField>

<ResponseField name="object" type="string">
  The object type
</ResponseField>

<ResponseField name="success" type="number">
  Indicates whether the call was successful. 1 if successful, 0 if not.
</ResponseField>

<RequestExample>

```bash cURL
curl --location --request DELETE 'https://api.stateset.com/api/purchase_order' \
--header 'Content-Type: application/json' \
--header 'Authorization: Token <token>' \
--data-raw '{
		"id": "po_1NXWPnCo6bFb1KQto6C8OWvE"
}'
```

```graphQL GraphQL
mutation deletePurchaseOrder ($id: String!) {
    delete_purchaseorder(where: {id: {_eq: $id}}) {
          affected_rows
        }
      }
```

```js Node.js
const purchaseorders = await stateset.purchaseorders.del({
  'po_1NXWPnCo6bFb1KQto6C8OWvE'
});
```

```py Python
purchaseorders = stateset.purchaseorders.del({
  'po_1NXWPnCo6bFb1KQto6C8OWvE'
})
```

```go Go
purchaseorders, err := stateset.purchaseorders.Del(
  "po_1NXWPnCo6bFb1KQto6C8OWvE"
)
```

```java Java
Return purchaseorders = stateset.purchaseorders.del(
  "po_1NXWPnCo6bFb1KQto6C8OWvE"
);
```

```js Javascript
const purchaseorders = await stateset.purchaseorders.del({
  'po_1NXWPnCo6bFb1KQto6C8OWvE'
});
```

```ruby Ruby
purchaseorders = Stateset::PurchaseOrder.del({
  'po_1NXWPnCo6bFb1KQto6C8OWvE'
})
```

```php PHP
$purchaseorders = $stateset->purchaseorders->del(
  'po_1NXWPnCo6bFb1KQto6C8OWvE'
);
```

```http HTTP
DELETE /api/purchaseorder HTTP/1.1
Host: api.stateset.com 
Content-Type: application/json
```

</RequestExample>

<ResponseExample>

```json Response
{
  "id": "po_1NXWPnCo6bFb1KQto6C8OWvE",
  "object": "purchaseorder",
  "deleted": true
}

```

</ResponseExample>
